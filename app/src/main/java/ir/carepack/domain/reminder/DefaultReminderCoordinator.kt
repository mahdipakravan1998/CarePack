...
class DefaultReminderCoordinator(
    private val scheduleSource: ReminderScheduleSource,
    ...
    override suspend fun currentStatus(): ReminderStatus {
        val preferenceState = preferenceStore
                .state.first()
    ...
    override suspend fun cancelAllOwnedReminderState() {
        operationLock.withGate {
            val scheduleAlarmKeys = scheduleSource
                    .getAllScheduleSeriesIds().map(
                        AlarmKey::forScheduleSeries,
                    ).toSet()

            val delayedAlarmKeys = snoozedReminderStore
                    .reminders.first()
                    .map {
                        it.alarmKey
                    }.toSet()
    ...
    private suspend fun handleAlarmFiredLocked(
        ...
        val notificationPermissionGranted = notificationPermissionGateway
                .isPermissionGranted()
        ...
        recordDiagnostic(
            ...
            occurrenceId = occurrenceId,
            outcome = notificationPermissionGranted
                    .toString(),
        )
        ...
        if (!notificationPermissionGranted) {
            recordDiagnostic(
                type = ReminderDiagnosticEventType
                        .NOTIFICATION_SKIPPED,
                occurrenceId = occurrenceId,
                outcome = "notification_permission_denied",
            )
        ...
        if (target == null) {
            recordDiagnostic(
                type = ReminderDiagnosticEventType
                        .NOTIFICATION_SKIPPED,
                occurrenceId = occurrenceId,
                outcome = "occurrence_not_eligible",
            )
        ...
        val notificationPosted = runPlatformOperation {
        ...
        val reconciliation = reconcileLocked(
                reason = ReconciliationReason
                        .ALARM_FIRED,
            )
        ...
        if (notificationPosted) {
            ...
            recordDiagnostic(
                type = ReminderDiagnosticEventType
                        .NOTIFICATION_FAILED,
                occurrenceId = target.occurrenceId,
                outcome = "notification_gateway_failed",
            )
        }
        ...
        return if (notificationPosted) {
            ...
            AlarmFireResult.NotificationFailure(
                    occurrenceId = occurrenceId,
                    reconciliation = reconciliation,
                )
        }
    ...
    private suspend fun remindLaterLocked(
        ...
        val decision = SnoozedReminderPolicy.create(
                occurrenceId = occurrenceId,
                now = now,
                remindAt = now.plusSeconds(
                        delayMinutes * SECONDS_PER_MINUTE,
                    ),
                occurrenceAlreadyReported = false,
                occurrenceActive = true,
            )

        val snoozedReminder = when (decision) {
        ...
        val canScheduleExactAlarms = exactAlarmCapabilityGateway
        ...
        recordDiagnostic(
            ...
            occurrenceId = occurrenceId,
            outcome = canScheduleExactAlarms
                    .toString(),
        )

        val mode = if (canScheduleExactAlarms) {
        ...
        val scheduledMode = scheduleWithFallback(
                target = target.copy(
                        alarmKey = snoozedReminder
                                .alarmKey,
                        scheduledAt = snoozedReminder
                                .remindAt,
                    ),
                preferredMode = mode,
            )
        ...
        return if (scheduledMode == null) {
            ...
            recordDiagnostic(
                type = ReminderDiagnosticEventType
                        .SNOOZE_SCHEDULED,
                occurrenceId = occurrenceId,
                alarmKey = snoozedReminder.alarmKey,
                deliveryMode = scheduledMode
    ...
    private suspend fun ignoredAlarmFire(
        ...
        return AlarmFireResult.Ignored(
            occurrenceId = occurrenceId,
            reason = reason,
            reconciliation = reconcileLocked(
                    reason = ReconciliationReason
                            .ALARM_FIRED,
                ),
        )
    ...
    private suspend fun reconcileLocked(
        ...
        val storedDelayedReminders = snoozedReminderStore
                .reminders.first()

        val delayedAlarmKeys = storedDelayedReminders
                .map {
                    it.alarmKey
                }.toSet()

        val allAlarmKeys = scheduleAlarmKeys +
                    delayedAlarmKeys

        if (
            initialStatus.availability == ReminderAvailability.DISABLED ||
            initialStatus.availability == ReminderAvailability
                .NOTIFICATION_PERMISSION_REQUIRED || initialStatus.availability ==
            ...
            val cancellationResult = cancelAlarmKeys(
                    alarmKeys = allAlarmKeys,
                )
            ...
            return createResult(
                ...
                scheduledCount = 0,
                cancelledCount = cancellationResult
                        .successfulCount,
                failedOperationCount = cancellationResult
                        .failedCount,
            )
        }
        ...
        val normalTargets = scheduleSource
                .getNextEligibleTargets(
                    now = now,
                )

        val delayedTargets = resolveDelayedTargets(
                reminders = storedDelayedReminders,
                now = now,
            )

        val targets = normalTargets +
                    delayedTargets

        val targetKeys = targets
                .map(
                    ReminderTarget::alarmKey,
                ).toSet()

        val staleKeys = allAlarmKeys -
                    targetKeys
        ...
        var exactRegistrationCount = 0

        var approximateRegistrationCount = 0

        var scheduleFailureCount = 0
        ...
        targets.forEach { target ->
            ...
            recordDiagnostic(
                type = ReminderDiagnosticEventType
                        .EXACT_ALARM_CAPABILITY_CHECKED,
                occurrenceId = target.occurrenceId,
                alarmKey = target.alarmKey,
                outcome = canScheduleExactAlarms
                        .toString(),
            )

            val preferredMode = if (canScheduleExactAlarms) {
                    AlarmDeliveryMode.EXACT
                } else {
                    recordDiagnostic(
                        type = ReminderDiagnosticEventType
            ...
            val registrationMode = scheduleWithFallback(
                    target = target,
            ...
            when (registrationMode) {
                ...
                AlarmDeliveryMode.APPROXIMATE -> {
                    approximateRegistrationCount += 1
                }
        ...
        return createResult(
            reason = reason,
            status = finalStatus,
            scheduledCount = exactRegistrationCount +
                        approximateRegistrationCount,
            cancelledCount = cancellationResult
                    .successfulCount,
            failedOperationCount = scheduleFailureCount +
                        cancellationResult.failedCount,
        )
    }
    ...
    private suspend fun resolveDelayedTargets(
        ...
        return reminders.mapNotNull { reminder ->
            ...
            if (target == null) {
                ...
                target.copy(
                    alarmKey = reminder.alarmKey,
                    scheduledAt = maxOfInstant(
                            first = reminder.remindAt,
                            second = now,
                        ),
                )
            }
    ...
    private suspend fun buildStatus(
        preferenceState: ReminderPreferenceState,
    ): ReminderStatus {
        val permissionGranted = notificationPermissionGateway
                .isPermissionGranted()

        recordDiagnostic(
            type = ReminderDiagnosticEventType
                    .NOTIFICATION_PERMISSION_CHECKED,
            outcome = permissionGranted
                    .toString(),
        )

        val hasActiveSchedule = scheduleSource
                .hasActiveSchedule()

        recordDiagnostic(
            type = ReminderDiagnosticEventType
                    .FUTURE_OCCURRENCE_CHECKED,
            outcome = hasActiveSchedule
                    .toString(),
        )

        val exactCapabilityGranted = if (
                preferenceState.remindersEnabled &&
                permissionGranted && hasActiveSchedule
            ) {
        ...
        return ReminderStatus(
            ...
            exactAlarmCapabilityGranted = exactCapabilityGranted,
    ...
    private fun scheduleWithFallback(
        ...
        val preferredSucceeded = runPlatformOperation {
                alarmGateway.schedule(
                    request = AlarmRequest(
                            alarmKey = target.alarmKey,
                            occurrenceId = target.occurrenceId,
                            triggerAt = target.scheduledAt,
                            deliveryMode = preferredMode,
                        ),
                )
            }
        ...
        if (
            preferredMode != AlarmDeliveryMode.EXACT
        ) {
            return null
        }
        ...
        recordDiagnostic(
            ...
            occurrenceId = target.occurrenceId,
            alarmKey = target.alarmKey,
            deliveryMode = ReminderDeliveryMode.APPROXIMATE,
        )

        val fallbackSucceeded = runPlatformOperation {
                alarmGateway.schedule(
                    request = AlarmRequest(
                            alarmKey = target.alarmKey,
                            occurrenceId = target.occurrenceId,
    ...
    private fun cancelAlarmKeys(
        ...
        alarmKeys.forEach { alarmKey ->
            val succeeded = runPlatformOperation {
                    alarmGateway.cancel(
                        alarmKey = alarmKey,
                    )
                }
