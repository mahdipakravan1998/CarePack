...
class AndroidNotificationGateway(
    ...
    private val diagnosticSink: ReminderDiagnosticSink = NoOpReminderDiagnosticSink,
    private val idRegistry: NotificationIdRegistry = PersistentNotificationIdRegistry(context),
) : NotificationGateway,
    ReminderTestNotificationGateway {

    private val applicationContext = context.applicationContext

    private val notificationManager = checkNotNull(
    ...
    override fun post(
        ...
        try {
            val publicNotification = buildGenericNotification(
                    visibility = NotificationCompat.VISIBILITY_PUBLIC,
                )

            val privateNotification = NotificationCompat.Builder(
                    applicationContext,
    ...
    override fun postTestReminder(
        ...
        val notification = NotificationCompat.Builder(
                ...
                ReminderNotificationContract.CHANNEL_ID,
                    ...
                    applicationContext.getString(
                        R.string.reminder_test_notification_title,
                    ),
                ).setContentText(
                    applicationContext.getString(
                        R.string.reminder_test_notification_body,
                    ),
                ).setCategory(NotificationCompat.CATEGORY_ALARM)
                .setPriority(NotificationCompat.PRIORITY_HIGH).setDefaults(
                    NotificationCompat.DEFAULT_SOUND or
                            NotificationCompat.DEFAULT_VIBRATE,
                ).setVibrate(REMINDER_VIBRATION_PATTERN)
                .setVisibility(
                ...
                .setShowWhen(true).setOnlyAlertOnce(false)
    ...
    override fun cancel(
        ...
        val notificationId = idRegistry.findExistingId(
    ...
    private fun createChannel() {
        val channel = NotificationChannel(
                ReminderNotificationContract.CHANNEL_ID,
                applicationContext.getString(
                    R.string.reminder_notification_channel_name,
                ),
    ...
    private fun createTestContentPendingIntent(): PendingIntent = PendingIntent.getActivity(
            applicationContext,
            ReminderNotificationContract.testContentRequestCode(),
            ReminderNotificationContract.createOpenReminderSettingsIntent(
                    applicationContext,
                ),
            PendingIntent.FLAG_UPDATE_CURRENT or
                    PendingIntent.FLAG_IMMUTABLE,
    ...
    private fun occurrenceNotificationId(
        occurrenceId: String,
    ): Int = idRegistry.idFor(
            namespace = OCCURRENCE_NAMESPACE,
            stableKey = occurrenceId,
        )

    private companion object {
        val REMINDER_VIBRATION_PATTERN = longArrayOf(0L, 500L, 250L, 500L)
