...
class AppReconciler(
    private val medicationDeletionCoordinator: MedicationDeletionCoordinator,
    ...
    ): AppReconciliationOutcome = operationGate.withGate {
    ...
    private suspend fun reconcileLocked(
        ...
        val medicationRecovery = medicationDeletionCoordinator
                .resumeIncompleteDeletionIfNeeded()

        if (
            medicationRecovery is MedicationDeletionRecoveryResult.Failed
        ) {
            return AppReconciliationOutcome.Failed(
                stage = AppOperationStage
                        .RECOVERING_MEDICATION_DELETION,
                failure = medicationRecovery.failure
                        ?: SafeAppFailure(
        ...
        val dataRecovery = dataDeletionCoordinator
                .resumeIncompleteDeletionIfNeeded()

        if (dataRecovery is DataDeletionResult.Failed) {
            return AppReconciliationOutcome.Failed(
                stage = AppOperationStage.RECOVERING_DELETE_ALL,
                failure = dataRecovery.failure
                        ?: SafeAppFailure(
                            kind = AppFailureKind.UNKNOWN,
                            stage = AppOperationStage
        ...
        val generation: GenerationSummary = try {
                occurrenceGenerator.guaranteeMaintenanceWindowForAll(
                        anchorDate = now.atZone(currentZone)
                                .toLocalDate(),
                        now = now,
                    )
                ...
                return AppReconciliationOutcome.Failed(
                    ...
                    failure = throwable.toSafeAppFailure(
                            AppOperationStage.MAINTAINING_OCCURRENCES,
                        ),
                )
            }
        ...
        return try {
            val reminderResult = reminderCoordinator.reconcile(reason)
            val partialFailure = reminderPreferenceStore.recordReconciliationHealth(
            ...
            val failure = throwable.toSafeAppFailure(
                    AppOperationStage.RECONCILING_REMINDERS,
                )
