...
internal fun ReminderTestCard(
    ...
    Card(
        ...
        Column(
            ...
            Text(
                text = stringResource(
                        R.string.reminder_test_description,
                    ),
                style = MaterialTheme
            ...
            Button(
                onClick = onSchedule,
                enabled = !state.isSchedulingTest,
                modifier = Modifier
                ...
                if (state.isSchedulingTest) {
                    ...
                    Text(
                        text = stringResource(
                                R.string.reminder_test_action,
                            ),
                    )
                }
            }

            val statusText = when (state.reminderTestStatus) {
                    ReminderTestUiStatus.IDLE -> null

                    ReminderTestUiStatus.SCHEDULED_EXACT ->
                        stringResource(
                            R.string.reminder_test_scheduled_exact,
                        )

                    ReminderTestUiStatus.SCHEDULED_APPROXIMATE ->
                        stringResource(
                            R.string.reminder_test_scheduled_approximate,
                        )

                    ReminderTestUiStatus.NOTIFICATION_PERMISSION_REQUIRED ->
                        stringResource(
                            R.string.reminder_test_permission_required,
                        )

                    ReminderTestUiStatus.SCHEDULING_UNAVAILABLE ->
                        stringResource(
                            R.string.reminder_test_scheduling_unavailable,
                        )
                }
            ...
            statusText?.let { message ->
                Text(
                    text = message,
                    color = when (state.reminderTestStatus) {
                            ReminderTestUiStatus.SCHEDULED_EXACT,
                            ReminderTestUiStatus.SCHEDULED_APPROXIMATE,
            ...
            Text(
                text = stringResource(
                        R.string.reminder_test_delivery_notice,
                    ),
                style = MaterialTheme
...
internal fun ReadinessSummaryCard(
    ...
    Card(
        ...
        Column(
            ...
            Text(
                text = stringResource(
                        R.string.reminder_readiness_title,
                    ),
                style = MaterialTheme
            ...
            Text(
                text = readiness
                        ?.message ?: stringResource(
                            R.string.reminder_readiness_loading,
                        ),
...
internal fun PermissionActionSection(
    ...
    if (
        ...
        TextButton(
            ...
            Text(
                text = stringResource(
                        R.string.continue_without_permissions,
                    ),
            )
        }
    }
    ...
    if (
        state.remindersEnabled && state
            .notificationPermissionState != NotificationPermissionUiState
            .DENIED && !state.hasActiveSchedule
    ) {
        OutlinedButton(
    ...
    if (
        state.remindersEnabled && state
            ...
            .DENIED && state.hasActiveSchedule &&
        state.readiness
            ?.exactAlarm == ExactAlarmReadiness
            .UNAVAILABLE) {
        Button(
        ...
        TextButton(
            ...
            Text(
                text = stringResource(
                        R.string.continue_with_approximate_reminders,
                    ),
            )
        }
    }
    ...
    Card(
        ...
        Row(
            ...
            Column(
                ...
                Text(
                    text = stringResource(
                            R.string.reminders_enabled_label,
                        ),
                    style = MaterialTheme
                            .typography.titleMedium,
                )
                ...
                Text(
                    text = stringResource(
                            R.string.reminders_enabled_description,
                        ),
                )
            }
...
internal fun BatteryGuidanceSection(
    ...
    val batteryText = when (
            ...
            null,
                -> {
                stringResource(
                    R.string.battery_optimization_unknown,
                )
            }
        }
    ...
    Card(
        ...
        Column(
            ...
            Text(
                text = guidance
                        ?.body ?: stringResource(
                            R.string.generic_oem_guidance_body,
                        ),
                modifier = Modifier.testTag(
                        "oem_guidance_body",
                    ),
...
internal fun reminderAvailabilityText(
    ...
    if (
        readiness?.approximateFallbackAvailable == true
    ) {
        return stringResource(
            R.string.reminder_mode_approximate,
        )
    }
    ...
    return stringResource(
        when (state.availability) {
            ReminderAvailability.DISABLED -> {
                R.string.reminder_mode_disabled
            }

            ReminderAvailability.NOTIFICATION_PERMISSION_REQUIRED -> {
                R.string.reminder_mode_notification_unavailable
            }

            ReminderAvailability.NO_ACTIVE_SCHEDULE -> {
                R.string.reminder_mode_no_active_schedule
            }

            ReminderAvailability.EXACT -> {
                R.string.reminder_mode_exact
            }
...
internal fun ReminderSettingsUiState.withPlatformReadiness(
    ...
    val notificationPermissionGranted = notificationPermissionState ==
                NotificationPermissionUiState.GRANTED || notificationPermissionState ==
                NotificationPermissionUiState.NOT_REQUIRED

    return copy(
        readiness = ReminderReadinessPolicy.evaluate(
                remindersEnabled = remindersEnabled,
                hasActiveSchedule = hasActiveSchedule,
                notificationRuntimePermissionRequired = notificationRuntimePermissionRequired,
                notificationPermissionGranted = notificationPermissionGranted,
                canScheduleExactAlarms = exactAlarmCapabilityGranted,
